...
Máme $w=a^e$. V lookarounde sa pumpuje nejaké $a^f$, v $*$ mimo neho $a^g$ (ak je na výber viac možností, stačí vybrať jednu, napr. tú najkratšiu). Potom lookaround matchuje $a^{e+h*f}$ a $*$ mimo neho $a^{e+i*g}$, pre ľubovoľné $h,i \in \N_0$. Nech $b = nsn(f,g)$, prienik týchto slov je $a^{e+b*j} \in L(\alpha), ~ \forall j \in \N_0$. Tvrdíme $b<e$. A naozaj, keďže $a^e \in L(\alpha)$ a zároveň lookahead aj $*$ určite niečo pumpovali, ich najmenšia možná zhoda je práve $b$.
...
\begin{df}
Triedu nad leregexami obohatenými o negatívny lookaround budeme nazývať LnEregex.
\end{df}

\begin{veta}
LnEregex je uzavretá na prienik, zjednotenie, iteráciu a zreťazenie.
\end{veta}
\begin{proof}
Prienik rovnako ako v \ref{prienik}, zjednotenie pomocou operácie $|$, iterácia pomocou $*$. Zreťazenie $L_1,L_2$ je $L_1L_2$, ak lne-regex na popis $L_1$ neobsahuje na konci znak $\mathdollar$. Inak nech tých znakov obsahuje $n$, potom pred prvý z nich lne-regex pre $L_2$ a pred všetky ostatné spätnú referenciu naň.
\end{proof}

Príklad na problematické zreťazenie. Nech $L_1=(?=L_{11}\mathdollar )L_{12}\mathdollar$ (predpokladáme, že $L_{11}$ ani $L_{12}$ neobsahujú $\mathdollar$), potom lne-regex pre $L_1L_2$ bude vyzerať naledovne:
$$\displaystyle (?=L_{11} \mathop(_kL_2\mathop)_k \mathdollar )L_{12} \backslash k\mathdollar$$

\begin{veta}
LnEregex je uzavretá na komplement.
\end{veta}
\begin{proof}
Nech $L_1 \in LnEregex$, potom $L_1^c  = \left( ?! L_1 \mathdollar \right) .* \mathdollar $.
\end{proof}
